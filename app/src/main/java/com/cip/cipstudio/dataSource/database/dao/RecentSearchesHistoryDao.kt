...
interface RecentSearchesHistoryDao {
    ...
    @Query("SELECT * FROM recent_searched_history WHERE INSTR(`query`, :query) > 0 AND user_id =:userId ORDER BY last_search DESC LIMIT :pageSize OFFSET :pageIndex * :pageSize")
    ...
    @Query("DELETE FROM recent_searched_history WHERE `query` =:query AND user_id =:userId" )
    ...
    @Query("SELECT COUNT(*) FROM recent_searched_history WHERE user_id =:userId")
    suspend fun isEmpty(userId : String) : Int
}